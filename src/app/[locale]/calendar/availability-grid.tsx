...
export default async function AvailabilityGrid({
  units,
  bookings,
}: {
  units: Unit[];
                ...
                {days.map((d) => {
                  ...
                  return (
                    <td
                      ...
                      className={`border border-slate-200 p-2 text-center dark:border-slate-700 ${booked
                          ? "bg-rose-200 dark:bg-rose-900/50"
                          : "bg-emerald-50 dark:bg-emerald-900/20"
                        }`}
                    >
                      <span className="text-xs">{booked ? t('booked') : t('free')}</span>
                    </td>
                  );
                })}
              </tr>
